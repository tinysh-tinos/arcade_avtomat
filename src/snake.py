...
def draw_eye(surface, cx, cy, direction):
    ...
    if dx != 0:
        ...
        pygame.draw.circle(surface, (20, 30, 20), (ex1, ey1), r)
        pygame.draw.circle(surface, (20, 30, 20), (ex2, ey2), r)
        pygame.draw.circle(surface, (255, 255, 255), (ex1 - 1, ey1 + dy), 2)
        pygame.draw.circle(surface, (255, 255, 255), (ex2 - 1, ey2 + dy), 2)
...
def draw_hud(score, best, speed):
    ...
    screen.blit(s1, (55, 45))
    screen.blit(s2, (55, 68))
    screen.blit(s3, (55, 108))


def game_over(score, best):
...
def draw_snake(snake, direction, time_ms):
    n = len(snake)
    for i, (x, y) in enumerate(snake):
        ...
        if i == 0:
            ...
            pygame.draw.rect(
                screen, (0, 0, 0, 0),
                (px - 2 - pulse, py - 2 - pulse, size + 4 + pulse * 2, size + 4 + pulse * 2),
                border_radius=12
            )
            glow = pygame.Surface((size + 16, size + 16), pygame.SRCALPHA)
            pygame.draw.rect(glow, (*SNAKE_HEAD, 60), glow.get_rect(), border_radius=14)
            screen.blit(glow, (px - 8, py - 8))

        draw_rounded_cell(screen, color, px, py, size, radius=8)

    hx, hy = snake[0]
    cx = hx * CELL + CELL // 2
    cy = hy * CELL + CELL // 2
    draw_eye(screen, cx, cy, direction)


def draw_food(food, time_ms):
    fx, fy = food
    cx = fx * CELL + CELL // 2
    cy = fy * CELL + CELL // 2
...
def main():
    ...
    while running:
        ...
        if not hasattr(main, "timer"):
            main.timer = 0
        main.timer += dt
        step_delay = 1000 / speed

        if main.timer >= step_delay:
            main.timer = 0
        ...
        draw_food(food, time_ms)
        draw_snake(snake, direction, time_ms)
        draw_hud(score, best, speed)
